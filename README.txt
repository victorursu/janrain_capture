...
* $scripts
  A multidimensional array of module and inline scripts
...
// Change the behavior of the default closeRecoverPassword function
function mymodule_janrain_capture_add_js($scripts) {
  foreach ($scripts['inline'] as &$value) {
    if (strpos($value, 'closeRecoverPassword')) {
      $value = 'function closeRecoverPassword() {
  // do something
}';
    }
  }
...
* $styles
  An array of module stylesheets
...
// Remove all styles from being queued
function mymodule_janrain_capture_add_css($styles) {
  return array();
}
